...
@section('title', 'ಸರ್ಕಾರಿ ಯೋಜನೆಗಳು ಮತ್ತು ಸಬ್ಸಿಡಿ — Karnataka Farmer Schemes')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <div class="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold mb-1">
                <span>🏛️ ಕೃಷಿ ಇಲಾಖೆ & ಸರ್ಕಾರದ ನೆರವು</span>
            </div>
            <h1 class="text-2xl sm:text-3xl font-black text-stone-900 tracking-tight">
                ಸರ್ಕಾರಿ ಯೋಜನೆಗಳು & ಸಬ್ಸಿಡಿ (Government Schemes)
            </h1>
            <p class="text-xs sm:text-sm text-stone-500">
                ಕರ್ನಾಟಕ ಮತ್ತು ಕೇಂದ್ರ ಸರ್ಕಾರದ ಕೃಷಿ ಸಬ್ಸಿಡಿಗಳು, ಯಂತ್ರೋಪಕರಣ ನೆರವು ಹಾಗೂ ಬೆಳೆ ವಿಮೆ ವಿವರಗಳು.
            </p>
        </div>
        ...
        <form method="GET" action="{{ route('farmer.schemes.index') }}" class="flex items-center gap-2">
            ...
            <div class="relative flex-1">
                ...
                <input type="text" 
                       name="search" 
                       value="{{ $search }}" 
                       placeholder="ಯೋಜನೆ ಹೆಸರು ಅಥವಾ ಇಲಾಖೆ ಹುಡುಕಿ (Search schemes, e.g. Tractor, PM-KISAN, ಹನಿ ನೀರಾವರಿ)..."
    ...
    @if($schemes->isEmpty())
        <div class="text-center py-16 bg-white rounded-2xl border border-stone-200 shadow-xs">
            ...
            <h3 class="text-base font-bold text-stone-800 mt-2">ಯಾವುದೇ ಯೋಜನೆಗಳು ಕಂಡುಬಂದಿಲ್ಲ</h3>
            <p class="text-xs text-stone-500 mt-1">ಬೇರೆ ಪದಗಳಿಂದ ಹುಡುಕಿ ಅಥವಾ ಫಿಲ್ಟರ್ ಬದಲಾಯಿಸಿ ನೋಡಿ.</p>
            ...
            @foreach($schemes as $scheme)
                <div class="bg-white rounded-2xl border border-stone-200/90 hover:border-emerald-500 hover:shadow-md transition flex flex-col p-5 group">
                    <div class="flex items-start justify-between gap-2 mb-3">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[11px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200">
                    ...
                    @if($scheme->benefits || $scheme->benefits_kn)
                        <div class="mt-4 p-2.5 rounded-xl bg-amber-50/70 border border-amber-200/60 text-xs text-amber-900">
                            <div class="font-bold text-[11px] text-amber-950 mb-0.5 flex items-center gap-1">
                                <span>💡 ಮುಖ್ಯ ಸೌಲಭ್ಯ:</span>
                            </div>
                            <div class="line-clamp-2">
                                {{ Str::limit(strip_tags($scheme->benefits_kn ?: $scheme->benefits), 100) }}
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="mt-5 pt-3 border-t border-stone-100 flex items-center justify-between">
                        <a href="{{ route('farmer.schemes.show', $scheme->slug) }}" class="inline-flex items-center gap-1 text-xs font-bold text-emerald-700 hover:text-emerald-800 transition">
                            <span>ಮಾಹಿತಿ & ಅರ್ಜಿ ವಿವರಗಳು &rarr;</span>
                        </a>
